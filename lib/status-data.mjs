...
export function buildStatusSnapshot(options) {
  ...
  const siteUrl = formatUrlFromDomain(process.env.NEXT_PUBLIC_SITE_URL) ?? FALLBACK_SITE_URL

  const mergedRuntime = mergeRuntimeOverrides(
    {
  ...
  const runtime = {
    ...mergedRuntime,
    deployment_url: formatUrlFromDomain(mergedRuntime.deployment_url),
    branch_url: formatUrlFromDomain(mergedRuntime.branch_url),
    production_url: formatUrlFromDomain(mergedRuntime.production_url) ?? siteUrl,
  }

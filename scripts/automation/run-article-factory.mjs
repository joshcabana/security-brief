...
async function main() {
  ...
  const payload = await requestJsonFromGitHubModels({
    ...
    maxTokens: 7000,
    systemPrompt:
      'You are the article generation engine for AI Security Brief. Return strict JSON only. No markdown fences. You will receive a weekly harvest source pack. Synthesize the findings into clear, authoritative briefings without inventing new technical dependencies.',
    userPrompt: [
      `Write 2 AI-assisted security briefings for ${context.effectiveDate}.`,
